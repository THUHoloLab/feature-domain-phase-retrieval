...
TV = 0.01; % denoising strength for TV denoising
...
for loop = 1:n_iters
    ...
    for sub_loop = 1:16
        disp(sub_loop)
        v = HIO(complex_TV(wavefront + u,TV,'isotropic'),1); % project to TV denoising
        wavefront = (wavefront + beta_0 * (v - u)) / (1 + beta_0);
        u = u + (wavefront - v);
        beta_0 = beta_0 * 2;
    end

...
%
% Datasets:
% rawImage_HeLa.mat                 Measurements of HeLa cell sample
...
for iLoop=1:loopNum
    ...
    objectRecoveryShift = objectRecoveryShift + gamaO * conj(CSRecovery).* ...
         (waveCSPlaneUpdate-waveCSPlane) ./...
         (alphaO.*max(max(abs(CSRecovery).^2)) + (1-alphaO).*(abs(CSRecovery)).^2);
    
    

    % Use rPIE algorithm to update the coded surface profile (optional)
    if iLoop>3
    CSRecovery = CSRecovery + gamaCS * conj(objectRecoveryShift).*...
         (waveCSPlaneUpdate-waveCSPlane) ./...
         (alphaCS.*max(max(abs(objectRecoveryShift).^2))+(1-alphaCS).*(abs(objectRecoveryShift)).^2);
    end
% 
    ...
    % objectRecoveryAmplitude = objectRecoveryRefocus(1+end/2-150*mag:end/2+150*mag,1+end/2-350*mag:end/2-50*mag);
    ...
    % objectRecoveryPhase = objectRecoveryRefocus(1+end/2-150*mag:end/2+150*mag,1+end/2-350*mag:end/2-50*mag);
    objectRecoveryPhase = objectRecoveryRefocus;
    edge0 = 5;
    imRawShow = imRaw(:,:,1);%(1+end/2-150:end/2+150,1+end/2-350:end/2-50,1);
    figure(2024);
    subplot(131);imshow(abs(CSRecovery),[]);title('Raw image')
    subplot(132);imshow(objectRecoveryAmplitude,[]);title('Refocused amplitude')
    subplot(133);imshow(objectRecoveryPhase,[]);title('Refocused phase');
    drawnow;
end 
